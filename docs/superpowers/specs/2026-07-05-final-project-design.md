...
Nâng cấp EduChatbot (Assignment 2) thành Final Project với: giao diện mới, governance chunk theo vai trò, chức năng gói trả phí (MoMo), đo lường token, thống kê/báo cáo, và benchmark model. Chia việc cho 4 thành viên để contribution GitHub đồng đều và trung thực.
...
| Biểu đồ | **Chart.js** |
| Thanh toán | **MoMo sandbox** (server→server tạo `payUrl` + return, ký HMAC-SHA256; creds test công khai), tách `IPaymentService` |
...
- **PaymentOrder** — `Id, UserId, PackageId, Amount, VnpTxnRef, Status(Pending/Paid/Failed), CreatedAt, PaidAt`. Đối soát MoMo → nguồn **doanh thu**.
...
### M4 — Gói + Thanh toán MoMo
- Học sinh xem gói → mua → redirect MoMo sandbox → return/IPN xác nhận → tạo/gia hạn `UserSubscription`, set `PaymentOrder=Paid`.
...
- `IPaymentService` → `MoMoService` (tách lớp, dễ đổi cổng khác).
...
- **BLL:** `PackageService, SubscriptionService, PaymentService(IPaymentService→MoMoService), TokenUsageService, ChunkConfigService, StatisticsService, BenchmarkService`. Đăng ký DI tập trung tại `ServiceCollectionExtensions.AddProjectDependencies`.
- **Presentation:** Pages mới (Package, Checkout, Statistics, Benchmark, ChunkConfig) + endpoint callback MoMo. SignalR tái dùng cho dashboard realtime.
...
| **B** | BLL | PackageService, SubscriptionService, MoMo PaymentService, TokenUsageService, ChunkConfigService |
...
1. M1 UI Tabler → 2. M2 chunk governance → 3. M4 gói + MoMo → 4. M5 model + token log → 5. M6 thống kê → 6. M3 hoàn thiện citation → 7. M7 benchmark.
